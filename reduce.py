...
class LL1Parser:
    def __init__(self):
        ...
        self.state_symbols, self.productions['standard'] = copy.deepcopy(LL1Run())
        self.input_symbols = ['(', ')', '+', '-', '*', '/', 'n', '$']
        tmp = self.productions.get('standard')
        self.first, self.follow = copy.deepcopy(getFirstandFollow(tmp, self.state_symbols))
        self.symbol_stack = SymbolStack('$E')
        self.table = AnalyseTable(self.productions, self.first, self.follow, self.input_symbols, self.state_symbols)
        self.inputs = Inputs()
        self.process()

    def print_a_line(self, char, info, report='正确'):
        text_output.insert('insert','%-30s' % self.symbol_stack.stack, 'tag2')
        text_output.insert('insert',"")
        text_output.insert('insert', '%-32s' % self.inputs.inputs, 'tag2')
        text_output.insert('insert', "")
        text_output.insert('insert', '%-8s' % char, 'tag2')
        text_output.insert('insert', "")
        text_output.insert('insert', '%-8s' % report, 'tag2')
        text_output.insert('insert', "")
        text_output.insert('insert', '%-24s' % info, 'tag2')
        text_output.insert('insert', "\n")
...
def getFirstandFollow(prodections,state_symbols):             #获取first集合
    ...
    resultFirst = copy.deepcopy(getFirst(resultFirst, prodections,state_symbols))
...
def getFirst_3(resultFirst, prodections,state_symbols):
    result =copy.deepcopy(resultFirst)
    #print(result)
    #print("#########")
    while(True):
...
def getFollow_1(resultFollow,First, prodections,state_symbols):
    FOLLOW = copy.deepcopy(resultFollow)
    for str in prodections:
        leftP = str.split("->")[0]
        rightP = str.split("->")[1]
        tmp = []
...
def getFollow_3(resultFollow,First, prodections,state_symbols):
    ...
    while(True):
        ...
        FOLLOW = copy.deepcopy(getFollow_1(FOLLOW,First,prodections,state_symbols))
